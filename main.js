...
function TrafficGame() {
	this.city = 'sf';			// actual city [london, sf, newYork, tokyo, sydney, budapest]
	...
	this.setImages = function() {
		...
		// trees:
      this.treeImages = [];
		this.treeImages[0] = UTIL.imageBySprite('tree1');
		this.treeImages[1] = UTIL.imageBySprite('tree2');
		this.treeImages[2] = UTIL.imageBySprite('tree3');
		this.treeImages[3] = UTIL.imageBySprite('tree4');
      ...
      // grass:
      this.grassImage = UTIL.imageBySprite('grass');
	   
      // dirt:
      this.dirtImage = UTIL.imageBySprite('dirt');
   };
	...
	this.animationRoutine = function() {
		...
		for (var row = 0; row < 9; row++) {
			for (var col = 0; col < 14; col++) {
				...
				if (t > 0) {
					...
					if (t == 1) engine.canvas.drawImage(this.dirtImage, x, y);
					if (t == 2) engine.canvas.drawImage(this.grassImage, x, y);
		...
		for (var i = 0; i < DATA.cities[this.city].trees.length; i++) {
         ...
         engine.canvas.drawImage(this.treeImages[f], x, y);
		}
		...
		// draw city names:
		if (this.city == 'sf') {
			UTIL.drawText('S A N', 658, 290, 11, 18);
			UTIL.drawText('FRANCISCO', 658, 325, 11, 18);
      }
		if (this.city == 'london') UTIL.drawText('L O N D O N', 3, 330, 8, 18);
		if (this.city == 'tokyo') UTIL.drawText('T O K Y O', 888, 590, 7, 18);
		...
		for (var key in this.trafficLights) {
			...
			for (var i = 0; i < 4; i++) {
				...
				var d = i % 2 ? 'h' : 'v';
				UTIL.rect(x + oX[i], y + oY[i], d == 'h' ? 48 : 24, d == 'v' ? 48 : 24, 0);
		...
		if (this.scene == 'menu') {
			...
			UTIL.drawText('G R I D L O C K', oX + 145, oY - 10, 1, 18, 170);
